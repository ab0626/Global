...
def test_gate_vetoes_thin_single_channel_evidence_but_keeps_corroborated_pairs() -> None:
    ...
    # ...and an untitled node can be held to two channels instead
    assert apply_gate(frame, relaxed)["gated"].to_list()[4] == 0.5
    strict_bridge = dataclasses.replace(relaxed, titleless_min_channels=2)
    assert apply_gate(frame, strict_bridge)["gated"].to_list()[4] == 0.0
    ...
    strict = ClusterSettings(cross_language_title_floor=0.65)
